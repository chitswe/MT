		...
		return (
			<div className="cs-grid" style={{position:'relative'}}>
				...
				<div className="cs-body" >
					{
						data.map(item=>(<GridCard onDoubleClick={onRowDoubleClick} isCardExpended={isCardExpended} isSelected={selected ? selected.indexOf(item[primaryKey])>-1 : false} key={item[primaryKey]} onSelectedChanged={this.handleRowSelectedChange.bind(this)} changeToCardAt={changeToCardAt} cardLabelWidth={cardLabelWidth} columns={columns} primaryKey={primaryKey} item={item}/>))
					}
					{
						loading? 
